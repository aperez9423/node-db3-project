...
- write SQL statements against the `northwind.db3` database. Once you have the correct SQL Statement for each query, write it inside the _queries.sql_ file under the corresponding comment. DONE
- configure Knex by creating a `db-config.js` file that consumes the `knexfile.js`. DONE
...
(Write the queries inside `./queries.sql`) DONE
